...
@click.option('--sort/--no-sort',
              default=True,
              help='Sort the structures by energy per atoms')
def view_res_ovito(files, sort):
    ...
    if sort:
        ...
        atoms_list = [
            atoms
            for _, atoms in sorted(zip(engs, atoms_list), key=lambda x: x[0])
        ]
...
def trim_tags(input_file, tag_to_remove):
    ...
    if trimmed is True:
        if in_section is False:
            ...
            click.echo(
                f"Did not find the end of the tag {tag_to_remove} - aborting")
...
@click.option('--cell-type', '-c', default='standard', type=click.Choice(['standard', 'primitive', 'primitive-standard']))
